...
public final class FlowerView extends View {
  ...
  public FlowerView(
      ...
      int petalThickness, int petalCount, float petalAlpha, float borderPadding,
      float centerPadding,
      int themeColor, int fadeColor,
      String text, float textSize, int textColor, float textAlpha, int textMarginTop,
      boolean textExpandWidth) {
  ...
  private void init(
    ...
    mIsExpandWidth = (!TextUtils.isEmpty(text) && textExpandWidth);
    ...
    if (mIsExpandWidth) {
      mBackgroundRect = new RectF(0, 0, mSize + mTextHeight + mTextMarginTop,
          mSize + mTextHeight + mTextMarginTop);
    ...
    mPetalCoordinates = calc
        .getSegmentsCoordinates(mSize, (int) (borderPadding * mSize), (int) (centerPadding * mSize),
            petalCount, mFinalSize);
    mPetalColors = calc
        .getSegmentsColors(themeColor, fadeColor, petalCount, (int) (petalAlpha * 255));
  }
  ...
  protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    if (mIsExpandWidth) {
      setMeasuredDimension(mSize + mTextHeight + mTextMarginTop,
          mSize + mTextHeight + mTextMarginTop);
    } else {
  ...
  protected void onDraw(Canvas canvas) {
    canvas.drawRoundRect(mBackgroundRect, mBackgroundCornerRadius, mBackgroundCornerRadius,
        mBackgroundPaint);
    ...
    for (int i = 0; i < mPetalCount; i++) {
      ...
      canvas.drawLine(coordinate.getStartX(), coordinate.getStartY(), coordinate.getEndX(),
          coordinate.getEndY(), mPetalPaint);
    }
    if (mText != null) {
