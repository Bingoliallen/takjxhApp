 ...
 *
 * @Author: libaibing
 * @Date: 2019-11-05 11:29
 * @Description:
 **/
public class XsfdpxAdapter  extends BaseRecyclerAdapter<TrainsBean.TrainListBean> {

    public XsfdpxAdapter(Context context) {
        super(context);
    ...
    @Override
    public void onBind(ViewHolder holder, final TrainsBean.TrainListBean item, final int position) {
        ...
        if(position==0){
            ...
            holder.getView(R.id.mR5).setVisibility(View.VISIBLE);

            ImageView iv5 = (ImageView) holder.getView(R.id.iv5);
            if (TextUtils.isEmpty(item.getCover())) {
                iv5.setImageResource(R.drawable.pic_defalt);
            }else{
                List<String> list= Arrays.asList(item.getCover().split(","));
                if(list.size()>0){
                    if(list.size()==1){
                        ImageWrapper.setImage(iv5,  item.getCover(), R.drawable.pic_defalt, ImageWrapper.CENTER_CROP);
                    }else {
                        ImageWrapper.setImage(iv5, list.get(0).toString(), R.drawable.pic_defalt, ImageWrapper.CENTER_CROP);
                    }
        ...
        holder.getView(R.id.mLM).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                XxspActivity.lunch((Activity) mContext, item.getId(), item.getVideo(),item.getTitle(),"2");
            }
        });
