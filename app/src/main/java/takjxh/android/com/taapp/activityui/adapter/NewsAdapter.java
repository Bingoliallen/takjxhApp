...
public class NewsAdapter extends BaseRecyclerAdapter<NewsBean.NewsListBean> {
    ...
    public NewsAdapter(Context context) {
        super(context);
        putItemLayoutId(R.layout.item_recycler_news);

    }


    @Override
    public void onBind(ViewHolder holder, final NewsBean.NewsListBean item, final int position) {
        ...
        if (TextUtils.isEmpty(item.getCover())) {
            ...
            if(list.size()>0){
                if(list.size()==1){
                    imgPath=list.get(0).toString();
                    ImageWrapper.setImage(iv_icon, list.get(0).toString(), R.drawable.pic_defalt, ImageWrapper.CENTER_CROP);
                }else{
        ...
        holder.getView(R.id.mLM).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                NewsDetailActivity.lunch((Activity) mContext, item.getId(), finalImgPath,item.getTitle());
            }
        });
