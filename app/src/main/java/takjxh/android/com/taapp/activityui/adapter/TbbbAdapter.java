...
public class TbbbAdapter extends BaseRecyclerAdapter<TaskDetailBean.Question1Bean> {


    public TbbbAdapter(Context context) {
        super(context);
        putItemLayoutId(R.layout.item_recycler_tbbb_list);

    }


    @Override
    public void onBind(ViewHolder holder, final TaskDetailBean.Question1Bean item, final int position) {
        ...
        } else if ("04".equals(item.getType())) {
            holder.getView(R.id.mLdx).setVisibility(View.GONE);
            holder.getView(R.id.mLrx).setVisibility(View.GONE);
            holder.getView(R.id.mL2).setVisibility(View.GONE);
    ...
    private void radioButtonL(final TaskDetailBean.Question1Bean item, final int position, final int pos) {
        boolean isto = false;
        boolean isSel = false;
        for (int a = 0; a < item.getItems().size(); a++) {
    ...
    private void checkBoxL(final TaskDetailBean.Question1Bean item, final int position, final int pos, final boolean isChecked) {
